...
for num, process_file in enumerate(shape_files_to_process): 
    os.subprocess(['ogr2ogr', '-f', 'PostgreSQL', 'PG:host=localhost port=5433 user=postgres password=password', './data_dir_{}/{}'.format(num, process_file), '-skipfailures', '-overwrite', '-progress', '-nln', 'data_table_{}'.format(num), '-nlt', 'MULTIPOLYGON', '-lco', 'GEOMETRY_NAME=geom_4326', '-lco', 'PRECISION=NO', '-t_srs', 'EPSG:4326', '--config', 'PG_USE_COPY', 'YES'], check=True)
